...
- `assets/uture-logo.svg` — Graphic mark (single-color "U" sparkle glyph).
- `assets/uture-text-logo.svg` — Wordmark "uture".
...
4. **Limit the orange.** Maximum one keyword OR one accent node per slide. If you need a second, rethink the slide.
5. **No emoji, no gradients, no shadows, no stock photos, no character illustrations, no full-bleed imagery.** When in doubt, leave it blank.
6. **Korean prose:** noun-form endings (`~한다`, `~된다`), no 여러분/저는, no exclamation marks, no marketing adjectives. See `README.md` § Content Fundamentals.
